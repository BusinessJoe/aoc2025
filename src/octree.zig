...
const Node = union(enum) {
    ...
    pub fn sqDistanceTo(self: Node, point: [3]u64) u64 {
        var closest_point: [3]u64 = undefined;
        switch (self) {
            .leaf => |leaf| closest_point = leaf,
...
pub const Octree = struct {
    ...
    pub fn init(allocator: Allocator, max_width: u64) error{OutOfMemory, Overflow}!Self {
        ...
        try nodes.append(allocator, Node{ .branch = BranchNode {
    ...
    pub fn contains(self: *Self, needle: [3]u64) bool {
        ...
        var node: Node = self.nodes.items[0];
        while (true) {
            switch (node) {
                .leaf => |point| return std.mem.eql(u64, &point, &needle),
                .branch => |branch| {
    ...
    pub fn insert(self: *Self, allocator: Allocator, point: [3]u64) !void {
            ...
            if (branch.branches[child_branch_idx]) |child_node_idx| {
                ...
                switch (child_node) {
                    ...
                    .leaf => |leaf| {
                        ...
                        if (std.mem.eql(u64, &leaf, &point)) return;

                        // This leaf needs to be replaced by a new branch
                        
                        var new_branch = Node { .branch = createSubBranch(branch.*, child_branch_idx) };
                        // The new branch needs to point to existing leaf
                        new_branch.branch.branches[new_branch.branch.indexOfBranchWithPoint(leaf)] = self.nodes.items.len;

                        self.nodes.items[child_node_idx] = new_branch;

                        // We need a new leaf node to represent the leaf that was previously here
                        const new_leaf = Node { .leaf = leaf };
                ...
                const new_leaf = Node { .leaf = point };
    ...
    pub const NNIterator = struct {
        ...
        pub fn init(tree: *const Octree, allocator: Allocator, target: [3]u64) !NNIterator {
        ...
        pub fn next(it: *NNIterator) ?[3]u64 {
...
test "contains when empty" {
    var octree = try Octree.init(testing.allocator, 16);
    defer octree.deinit(testing.allocator);
...
test "nearest neighbour iterator" {
    ...
    try octree.insert(testing.allocator, .{0, 0, 0});
    try octree.insert(testing.allocator, .{1, 0, 0});
    try octree.insert(testing.allocator, .{2, 0, 0});
    try octree.insert(testing.allocator, .{3, 0, 0});
    try octree.insert(testing.allocator, .{4, 0, 0});
    try octree.insert(testing.allocator, .{5, 0, 0});

    var iter = try octree.iterator(testing.allocator, .{10, 0, 0});
    defer iter.deinit(testing.allocator);

    try testing.expectEqual(.{5, 0, 0}, iter.next());
    try testing.expectEqual(.{4, 0, 0}, iter.next());
    try testing.expectEqual(.{3, 0, 0}, iter.next());
    try testing.expectEqual(.{2, 0, 0}, iter.next());
    try testing.expectEqual(.{1, 0, 0}, iter.next());
    try testing.expectEqual(.{0, 0, 0}, iter.next());
